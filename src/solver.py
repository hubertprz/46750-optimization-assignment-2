...
def solve_network(Network: DistributionNetwork, B, dr, Y, OutputFlag=0):
    """Solves a Distribution Network.

    Args:
        Network (DistributionNetwork): Defined Distribution Network
        B (float): Annual budget
        dr (float): Discount rate
        Y (int): Year (ONLY FOR MODEL 1). Used for cost discounting.
        OutputFlag (int, optional): _description_. Defaults to 0.

    Returns:
        dict: Dictionary with values of objective function and decision variables.
    """
    # ---------------
    #  Index sets: Utilized in later calculations
    # ---------------
    ...
    C_R = 200 # Cost of capacity reinforcement (assume equal for every substation)
    ...
    z = model.addVars(S, lb=0, vtype=GRB.INTEGER, name="z") # NEW
    ...
    #   Node assigned to a substation y[i,s] = 1 only if substation is active w[s] = 1
    for i in N:
        for s in S:
            model.addConstr(y[i,s] <= w[s], name=f"y_le_w_{i}_{s}")

    #   (3) Substation node assignment: assigned to self if activated
    for s in S:
    ...
    #   (4a) Existing substations activation
    ...
    #   (4b) Distribution lines activation
    #   If a substation was activated, its feeder line is forced to stay active
    #   Thinking about deleting this one...
    for (i,j,s) in Network.A_activated:
        model.addConstr(x[i,j,s] == 1, name="x_act")
    ...
    for s in S:
        model.addConstr(r[s] <= (Network.SUBSTATIONS[s-1].capacity + R * z[s]) * w[s], name=f"capacity_{s}") #NEW
        #model.addConstr(r[s] <= (Network.SUBSTATIONS[s-1].capacity) * w[s], name=f"capacity_{s}") #OLD
    ...
    #   (9) Radiality: each non-substation node has exactly one parent per assigned substation
    for s in S:
        for i in D:
    ...
    #   (10) Tree size: arcs = nodes assigned - w[s]
    for s in S:
    ...
    edge_term = quicksum(Network.edge_cost[(min(i,j),max(i,j))]*x[i,j,s] for (i,j) in Network.A for s in S)
    reinf_term = quicksum(C_R * z[s] for s in S)
    model.addConstr(fix_term + edge_term + reinf_term <= B, name=f"budget_{Y}")
